...
func (sh *saramaCgHandler) Setup(session sarama.ConsumerGroupSession) error {
	log.Event(session.Context(), "Sarama consumerGroup session Setup OK. ConsumerGroup is Ready", log.INFO, log.Data{"memberID": session.MemberID(), "claims": session.Claims()})
...
func (sh *saramaCgHandler) ConsumeClaim(session sarama.ConsumerGroupSession, claim sarama.ConsumerGroupClaim) error {
	...
	log.Event(sh.ctx, "+++ ConsumeClaim START")
	...
	for message := range claim.Messages() {
		...
		default:
			...
			return sh.consumeMessage(SaramaMessage{message, session})
		}
	}

	log.Event(sh.ctx, "+++ ConsumeClaim FINISH")
	return nil
}
